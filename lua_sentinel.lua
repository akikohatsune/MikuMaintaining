...
local function logger(level, msg)
  log.write(config.log_path, level, msg)
end

-- Self-check
local required = { "iptables", "ps", "stat" }
...
local function loop()
  while true do
    ...
    -- Health Check
    local health_interval = (config.health and config.health.interval) or config.loop_interval
    ...
    -- Janitor
    local janitor_interval = (config.janitor and config.janitor.interval) or 1800
    if now - last_janitor >= janitor_interval then
